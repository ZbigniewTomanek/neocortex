...
| Snapshot | observed snapshot path or `NOT_MEASURED` |
...
| `admin_jobs` | Saved admin API responses | Job ids, task names, statuses, and safe arguments |
...
Remove episode text, `episode_text`, `domain_hint`, prompts, model output, credentials, and authorization headers from saved admin responses.
...
|---|---|
| Source | Fixed corpus path and stable episode locator |
...
| Domains | Accepted domain slugs and proposal status |
...
Use counts, stable database ids, domain slugs, type names, relation types, action labels, and one-way hashes as safe values.

Use an entity name only when the safe-value filter marks it as non-sensitive. Use a one-way hash for every other name.

Do not include episode text, prompts, hidden reasoning, raw model output, secrets, email addresses, or sensitive audit fields.
...
Validate the JSON report against `qwen-parsing-report.schema.json`. Validate the fixed episode set for `E01` through `E28`.

Validate source links for every measured field. Validate that the complete episode set has no invented rows.
