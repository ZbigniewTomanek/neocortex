...
| 13 | Repository-wide action-audit privacy acceptance | Stage 4/6 interrupted arm | high | The interrupted local arm exposed source/model-derived strings in action records despite extraction-side corrections; partial evidence cannot be certified | Audit every `action_log=True` path, remove raw source/model-derived values, run adversarial privacy tests, obtain independent acceptance, then resume Stage 6 | IN_PROGRESS |
