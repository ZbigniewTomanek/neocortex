...
### D36: Keep Stage 6b repairs minimal, local-only, opaque, and fail-closed
**Date**: 2026-09-06 - **Stage**: 6b - **Type**: IMPLEMENTATION / PROVENANCE
**Chosen**: Apply the smallest bounded repairs supported by the isolation evidence: set `parallel_tool_calls=false` only for local librarian requests, normalize extraction correlation values to the code-owned `extract-[0-9a-f]{32}` format while preserving valid values across retries, and refuse canonical metrics after any unproven librarian failure that could have mutated the graph.
**Rationale**: The deterministic PydanticAI regression proved that a complete returned tool-call batch is checked before execution at the tool-call limit; increasing or removing the limit would hide the failure rather than diagnose it. The local E04/E05 probes exercised only serial three-agent extraction and completed without librarian failures, while the historical full diagnostic included domain classification and remains invalid. Strict opaque ids repair routed provenance without exposing source or domain data. The fail-closed gate prevents partial mutations from becoming a quality result without direct cleanliness or rollback proof.
**Evidence**: `docs/plans/33-local-qwen-migration/resources/isolation-attribution.md`; `docs/plans/33-local-qwen-migration/resources/isolation-evidence-20260906.json`; deterministic regression in `tests/test_librarian_mutation_tools.py`; correlation tests in `tests/unit/test_extraction_correlation.py`, `tests/test_jobs.py`, and `tests/unit/test_admin_jobs.py`; metrics gate test in `tests/unit/test_measurement_harness.py`.
**Constraints**: Hosted model settings, thresholds, corpus assumptions, privacy rules, and user preference for low local effort are unchanged. A fresh full Stage 6 arm is required; backlog items 12 and 14 remain `IN_PROGRESS` until terminal reliability and collision-free provenance are measured there.

### D37: Block Stage 6 on external Spark endpoint reachability
**Date**: 2026-09-06 - **Stage**: 6 - **Type**: PREFLIGHT / BLOCKER
**Chosen**: Record backlog item 15 for Spark Tailscale / local model endpoint reachability and block Stage 6 on that item alone.
