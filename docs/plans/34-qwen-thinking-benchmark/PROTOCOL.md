...
Rendered by `init_run.sh` on 2026-09-11 for the run `34-qwen-thinking-benchmark`. Preset `custom`: review placement `review-each-stage-after`, review budget `one-review-one-fix`, stop policy `never-stop`.
...
|---|---|---|---|
| Orchestrator | coordinator | `current-session` | this session |
| Implementer | strong | `claude/opus` | effort `high`, started by the orchestrator's harness |
| Reviewer | strong | `claude/opus` | effort `high`, started by the orchestrator's harness |
| Helper | light | `claude/sonnet` | effort `medium`, started by the orchestrator's harness |
...
One review, one fix, no re-review.

- **Fix round 1.** The complete blocking set in one assignment. The implementer reruns every affected check. Inspect the diff against every finding, including the ones with no deterministic check. Set `repair.rounds: 1` and `repair.used: true`.
- Accept on deterministic evidence, or block the stage. Failures found by the checks may be fixed and checked again; that is verification, not review. Do not send the fixed work to a reviewer.
